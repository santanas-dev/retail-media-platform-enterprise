...
**Last updated:** 2026-08-25 (CAMPAIGN-PERMISSION-SPLIT-001 — бандл `stand-c088111` выкачен, authorization debt закрыт)

**Next Active Workstream:** **LOCAL-DEV-STAND-001-FU-IDENTITY-SMOKE** — закрыть долги инструментов стенда (STAND-IDENTITY-SWITCH, UI-SMOKE-VS-STAND). Затем возврат к **PORTAL-UX-POLISH-001A1a**.
...
- State/Docs SHA: `0908c91` (develop — LOCAL-DEV-STAND-001 stand tooling). Stand bundle SHA **`c088111`** (`stand-c088111`, CAMPAIGN-PERMISSION-SPLIT-001; предыдущие — `da8d514`, `4635e72`). Payload SHA `8ad0228` unchanged — pilot/production release не менялся, `v0.11.1` не трогался.
...
- **STAND-IDENTITY-SWITCH (P2).** `version_identity()` в `local_stand.py` переключает только `RMP_VERSION`/`RMP_GIT_SHA`; `RMP_SCHEMA_HEAD` и `RMP_BUILD_TIME` остаются от предыдущего релиза, и `/version` врёт до ручной правки `.env.stand` — правил вручную оба раза (035 и 036). Нужно переносить schema head и build time из lock и класть schema head в lock на этапе сборки.
- **UI-SMOKE-VS-STAND (P2).** Набор написан под phase1/dev-компоуз и небезопасен для общего стенда: удаляет накопленные smoke-аккаунты, может назначить роль seed-пользователю, жёстко зашивает dev-пароли и `http://localhost:8000`, предполагает `CREATIVE_AUTO_APPROVE_UPLOADS=true`.
- **Откат стенда после миграции.** Образ предыдущего релиза не знает новой ревизии: `alembic upgrade head` падает с `Can't locate revision identified by '<head>'` (проверено экспериментально для 035). Откат одних образов недостаточен — нужен restore снятого дампа. Перед каждым update снимается `pg_dump` с контрольной суммой.
