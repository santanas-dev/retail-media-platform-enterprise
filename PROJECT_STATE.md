...
**Last updated:** 2026-08-25 (AUTHZ-CROSS-PORTAL-001 — RLS-утечка на дочерних таблицах кампании)

**Next Active Workstream:** **решение владельца по выкатке AUTHZ-CROSS-PORTAL-001 на стенд** (нужен новый образ control-api + миграция 035). После этого — возврат к **OWNER-UX-DECISION-001** и далее PORTAL-UX-POLISH.
...
- **⚠️ Стенд остаётся уязвим.** Миграция 035 применена только к одноразовой тестовой БД; стенд `rmp-local-stand` не трогался, образы не пересобирались и не публиковались. Для устранения на стенде нужен **новый образ control-api + прогон миграции** — это mutation пакета/стенда, требуется явное разрешение владельца.
