...
async def device_onboard(
    ...
    if code.status == "used":
        # Idempotent: same fingerprint → return existing device
    ...
    if code.status == "expired" or code.expires_at < now:
    ...
    # Check: same fingerprint already bound to a DIFFERENT code → reject
    existing_device = await repository.get_device_by_fingerprint(db, body.hardware_fingerprint)
    if existing_device:
        # Same code + same existing device → idempotent
        if code.physical_device_id and str(code.physical_device_id) == str(existing_device.id):
        ...
        # Different code → reject (fingerprint already bound)
        raise HTTPException(
            status_code=403,
            detail={"code": "FINGERPRINT_CONFLICT", "message": "Hardware fingerprint already registered"},
        )
    ...
    # Consume the code
    await repository.consume_onboarding_code(db, code, device, body.hardware_fingerprint)
...
async def create_device_code(
    ...
    Requires: active user with system_admin/security_admin role.
