...
**⚠️ §24 — ПЕРЕПОДТВЕРЖДЕНИЕ (2026-07-18):** выяснилось, что EDGE-001/002/003/004
(onboarding, manifest+подпись, PoP, heartbeat) уже сделаны и зелёные как КСО-вертикаль.
«Каркас сначала» теперь = рефактор 4 готовых задач в ядро+адаптер (churn проверенного
кода без 2-го канала). Рекомендация обновлена в пользу прагматики (контракты готовы →
оркестрацию при 2-м канале). Решение ниже действует до переподтверждения владельцем.

**§24 (первоначально выбрано): каркас каналов сначала — буквально по ТЗ §24/§25.**
...
Архитектура — по решению §24 (см. §5.1 — требует переподтверждения).
