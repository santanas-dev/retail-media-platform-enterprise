...
Заменён canonical cutover RM-GOV-005 (2026-08-26). Не реализовывать отсюда.

Куда переехало содержание:
  последовательность работ  -> docs/product/roadmap.yaml (sequencing SSOT)
  матрица зрелости, сводка,
  решения владельца, blocked  -> docs/product/generated/roadmap.generated.md (проекция)
  UX backlog PORTAL-UX-POLISH -> docs/product/portal-ux-polish.md
  честные проценты            -> docs/product/roadmap-methodology.md
  технические направления     -> docs/product/tech-directions.md
  бизнес-карта по ролям       -> docs/product/business-map-by-role.md

Известный дефект этого документа, зафиксированный при cutover: раздел «Сводка
(посчитано программно)» заявлял «backend/service без UI-journey: 10» при 13 в
feature-registry.yaml и в собственном заголовке раздела. Проекция считает это число
из registry, поэтому расхождение больше не воспроизводится.
-->
