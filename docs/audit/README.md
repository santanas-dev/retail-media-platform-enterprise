...
| `2026-08-31-claude-stage-c-ag-artifacts.md` | Стадия C: 10 артефактов AG — 13 файлов candidate/prepared (OD-043) без приёмки; C-задачи остаются planned до Gate-S; гейт DEP-NOT-CLOSED | `develop @ 06ae22e` + рабочее дерево | не закоммичено |
| `2026-08-31-claude-rm-tech-210-rls-device-routes.md` | RM-TECH-210 (стадия S): дефект RLS-CONTEXT-DEVICE-001 воспроизведён под ролью приложения и починен адаптивно (миграция 037 bootstrap по коду/fingerprint, set_rls_context, allowlist снят); targeted 97, behavioral 477/0 | `develop @ 236bda4` + рабочее дерево | не закоммичено; ждёт CI и owner gate device_contract |
| `2026-08-31-claude-rm-stab-006-journeys-registry.md` | RM-STAB-006 (стадия S): journeys.yaml нормативного формата (79, selectors из smoke, Happy-path N), валидатор check-journey-spec.py --strict PASS, self-test 11/11; OD-044 (D1–D3) записан, RM-STAB-003 verification | `develop @ 236bda4` + рабочее дерево | не закоммичено |
