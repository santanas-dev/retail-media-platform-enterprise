...
| Revision | `draft-2026-08-26-r40`; увеличивается при каждом содержательном изменении |
...
юридический статус отчёта, политика advertiser API-keys (scope, rotation, revoke либо
явное исключение из первой очереди), пилотная шкала `KSO → 10 → 100 → 500 → network`.
...
| REQ-BIZ-001…010 | §1.2, §3, §6.2–6.3, §22.1–22.6, §22.12 | бизнес-цикл, flight windows, типы/приоритеты, inventory, eligibility, SLA, underdelivery/make-good, quality report, financial reporting, workflow closure |
...
| REQ-OPS-001…008 | §5, §9, §13, §17, §22.7, §22.9–22.10 | device health/commands, bounded cache/offline lifecycle, emergency governance, SLO, HA, DR, rollout, flags, monitoring, incidents, runbooks |
...
| REQ-CONT-001 | Creative/rendition QA | format/MIME/codec/size/security/channel constraints, SHA-256, preview реального профиля и поведение hide-on-touch для УКМ4 с визуальной проверкой на 75% области |
| REQ-CONT-002 | Immutable media history | version, retention, logical delete |
| REQ-BIZ-001 | Placement/inventory calculation | формулы capacity/reserved/sold/free и forecast показов/контактов по периоду, времени, географии и channel/surface; conflict включает расписание, долю рекламного времени, priority и campaign limits; sold-out предлагает альтернативные магазины/периоды/слоты; пример placement: 500 магазинов, 08:00–12:00, 10 секунд, priority 50 |
| REQ-BIZ-002 | Commercial lifecycle | advertiser/brand/order/contract basis, campaign period/goal/limits/budget-or-volume/responsible/status, placement frequency/constraints, tariff/discount/package/bonus/compensation; versioned lifecycle and audit |
...
| REQ-BIZ-009 | Financial reporting boundary | плановая стоимость/выручка (если разрешено хранить) по campaign/branch/store/period/inventory type; paid/internal/filler/bonus/compensation разделены, tariff/discount version фиксируется и не меняет закрытые расчёты |
| REQ-BIZ-010 | Campaign workflow closure | для каждого шага lifecycle назначены role/owner, допустимые действия, SLA/deadline и transition criteria; `completed/closed` разрешён только после итогового plan/fact отчёта и фиксации причин отклонений |
| REQ-SEC-001 | Identity and MFA | AD/LDAP или корпоративный SSO; локальная учётная запись только как резервная admin break-glass; MFA для критичных ролей, session policy, запрет shared accounts и отдельное право для изменения approved campaigns |
...
| REQ-OPS-007 | Emergency control | только ограниченные роли с MFA и обязательной причиной; scope/priority/progress по устройствам, partial-result, audit и безопасный resume |
...
| REQ-INT-001 | УКМ4/чековые данные | batch/API/ETL contract; оценочный reach вычисляется по утверждённой формуле (playback × число чеков/транзакций за магазин/период), не влияет на кассовые/фискальные операции, а в отчётах отделён от фактических показов |
...
| REQ-DATA-001 | Data ownership and lifecycle | dictionary, lineage, PII, retention, archive/delete; PoP hot 12–18 мес., PoP/report archive 3–5 лет при споре/требовании, user/system audit ≥3 лет, technical logs 90–180 дней, creatives 1–3 года — точные сроки утверждают owner/legal; advertiser isolation; export только по scope/permission, с audit и лимитами периода/объёма |
...
и retention. Базовые окна до owner/legal решения: PoP hot 12–18 месяцев, архив PoP и итоговых
отчётов 3–5 лет при споре/требовании, audit не менее 3 лет, технические логи 90–180 дней,
креативы завершённых кампаний 1–3 года. Каждая выгрузка проверяет scope/permission, журналируется и ограничивается
...
| §6 Роли/кампании/инвентарь | REQ-BIZ-001, REQ-BIZ-003, REQ-BIZ-005, REQ-BIZ-006, REQ-BIZ-007, REQ-BIZ-008, REQ-BIZ-010, REQ-CORE-003, REQ-UX-001, REQ-SEC-002 | role matrix + formulas + workflow/underdelivery policy + journeys |
...
| §12 Reports | REQ-BIZ-001, REQ-BIZ-002, REQ-BIZ-004, REQ-BIZ-008, REQ-BIZ-009, REQ-BIZ-010, REQ-POP-001, REQ-UX-001 | report schema + plan/fact/channel-quality/underdelivery/financial/closure evidence |
...
| US-FIN-001 | J-FIN-001 | REQ-BIZ-002, REQ-BIZ-009 |
...
| AC-18 | Нормативные фразы самого драфта (эвристический скан текущей версии находит 89 строк-кандидатов, включая заголовки и пояснения) не имеют inline-ссылки на REQ-ID; связь существует только косвенно через разделы. | Классифицировать кандидаты, исключить заголовки/пояснения, а для каждой нормативной фразы добавить REQ-ID либо вынести её в атомарный каталог; проверять reverse-traceability автоматически. |
...
| AC-36 | §4 допускает API-ключи рекламодателя, но драфт не фиксировал, входят ли они в первую очередь и какие security controls обязательны. | Принять decision по advertiser API access: включить versioned scoped keys с rotation/revoke/audit либо явно исключить до отдельной задачи. |
...
До закрытия AC-01…AC-39 документ остаётся `DRAFT/REVIEW`; текст без артефакта и evidence не считается покрытием.
...
обязанностей. Текущие 66 REQ и карта 25 разделов не доказывают, что все 149 строк
