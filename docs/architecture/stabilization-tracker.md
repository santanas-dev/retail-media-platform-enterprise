...
| S-007 | PoP / reporting | P3 | 🔒 locked | — | ADR-017 accepted: PoP source (runtime only), ingestion endpoint (`POST /api/v1/pop/batch`, device JWT, event_id dedup), validation (quarantine for unknown manifests, clock drift ±5min), storage (PostgreSQL `pop_events_raw` + `pop_dedup_index`, ClickHouse deferred), reporting (only accepted successes, no fallback/duplicates/synthesized), outbox integration, audit trail, 9 behavioral proof requirements. Phase split: 4.3b (schema) → 4.3c (ingestion) → 4.3d (reporting) → 4.3e (views) | Implement 4.3b persistence schema |
