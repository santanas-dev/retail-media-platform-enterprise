...
| S-007 | PoP / reporting | P3 | 🔒 locked (4.3a) / ✅ 4.3b done | — | ADR-017 accepted: PoP source (runtime only), ingestion endpoint (`POST /api/v1/pop/batch`, device JWT, event_id dedup), validation (quarantine for unknown manifests, clock drift ±5min), storage (PostgreSQL `pop_events_raw` + `pop_dedup_index` + `pop_ingestion_batches`, ClickHouse deferred), reporting (only accepted successes, no fallback/duplicates/synthesized), outbox integration, audit trail, 12 behavioral proof requirements. **4.3b:** Migration 009 (3 tables + constraints + indexes), ORM models (PopEventRaw, PopDedupIndex, PopIngestionBatch), repository helpers (record_pop_raw_event, insert_pop_dedup_key, is_pop_event_duplicate, accept_pop_event, quarantine_pop_event, expire_pop_quarantine_events — caller-owned tx, no commit/ClickHouse/NATS). 23 unit + 10 behavioral tests. Phase split: 4.3b (schema) → 4.3c (ingestion) → 4.3d (reporting) → 4.3e (views) | Implement 4.3c ingestion endpoint |
