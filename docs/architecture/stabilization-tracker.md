...
**Current phase:** 4.1c (Campaign Mutation Foundation)
...
| S-005 | Campaign approval workflow | P2 | open | — | ADR-015 §approval locked; `campaigns.approve` permission seeded | Implement submit → approve/reject flow per ADR-015 transitions |
| S-006 | Delivery / manifest generation | P2 | open | — | ADR-012, ADR-013 locked; `manifest_v1.schema.json` defined | Resolve hierarchy → surfaces → manifest JSON |
