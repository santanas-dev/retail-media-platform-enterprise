...
| S-006d | Device gateway manifest endpoint | P2 | ✅ done | — | `apps/device-gateway/main.py`: `GET /api/v1/device/manifest/latest` with device JWT auth, device status check, ETag/If-None-Match, response shape alignment with `generate_manifest_json()`/`manifest_v1.schema.json`, no direct SQL in router, store-orphan detection. 10 unit + 7 behavioral tests. Commits: `c34d5fa` + `c8a369e` + `08b099e`. | Phase 4.2e: runtime simulator behavioral tests |
