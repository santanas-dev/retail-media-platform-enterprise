...
| S-006c | Manifest generator worker skeleton | P2 | ✅ done | — | packages/domain/delivery.py (723 lines): eligibility, target resolution, manifest JSON gen, persistence, outbox. 22 unit + 10 behavioral tests. Plan idempotency enforced. Note: split into sub-modules (eligibility/resolution/manifest/generation) before exceeding 1000 lines or Phase 4.2d. | Phase 4.2d: device gateway delivery endpoint |
