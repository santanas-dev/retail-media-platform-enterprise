...
- Campaign flight windows (`campaign_flights.start_at`..`end_at`) MUST
  fall within the contract validity window
  (`advertiser_contracts.valid_from`..`valid_until`).  If the contract
  has no `valid_until` (indefinite), only `valid_from ≤ flight.start_at`
  is enforced.  Enforcement at campaign creation/submit time — API
  rejects campaigns whose flights extend beyond the contract window
  (422).
...
Placements target a location in the hierarchy.  Exactly one targeting
level is required — surface (exact), store, cluster, or branch (broad).
The manifest generator resolves broad targets to concrete surface IDs at
generation time.  Placements never reference `physical_device_id`.

| Column | Type | Notes |
|--------|------|-------|
| `id` | UUID PK | |
| `campaign_id` | UUID FK NOT NULL | |
| `display_surface_id` | UUID FK nullable | Exact surface targeting |
| `store_id` | UUID FK nullable | Store-level targeting |
| `cluster_id` | UUID FK nullable | Cluster-level targeting |
| `branch_id` | UUID FK nullable | Branch-level targeting |
...
`cluster_id`, or `branch_id` must be non-null.  A placement with only
`store_id` targets all surfaces in that store; with only `branch_id`,
all surfaces in that branch — resolved at manifest generation time.

**Rationale:** physical devices can have multiple surfaces (dual-screen
KSO, LED panel array, ESL shelf array).  Targeting at surface level or
above decouples campaign logic from hardware topology.
...
| `approved` | `scheduled` | internal or system | `start_at`/`end_at` validated; `start_at` may be future |
| `approved` | `rejected` | internal | Revoke approval (reason required) |
| `approved` | `draft` | advertiser | Unlock for edits |
| `scheduled` | `active` | system (auto) | `start_at ≤ NOW()` AND approval record exists AND not paused |
| `scheduled` | `paused` | internal/advertiser | Manual pause before `start_at` |
| `paused` | `scheduled` | internal/advertiser | Resume while `start_at` is still in the future (before delivery begins) |
| `paused` | `active` | internal/advertiser | Resume after `start_at ≤ NOW()` (delivery window is open); resumes at previous priority |
| `active` | `paused` | internal/advertiser | Manual pause within delivery window |
...
Placements target a location in the hierarchy — exactly one level must
be set (at-least-one constraint):

- `display_surface_id` — exact surface targeting (narrow)
- `store_id` — all surfaces in a store (broad)
- `cluster_id` — all surfaces in a cluster (broad)
- `branch_id` — all surfaces in a branch (broad)

Broad targets are resolved at manifest generation time to a concrete
set of `display_surface_ids`.  The manifest contains only resolved
surface IDs — no hierarchy references.  Placements never reference
`physical_device_id`.

### 5. Placement and Surface Model

Placements target a location in the hierarchy — surface (narrow), store,
cluster, or branch (broad).  They never target `physical_device_id`
directly.
